...
- Tomcat : Apache Softeware Foundation에서 Servlet과 JSP를 통한 동적인 웹 문서를 처리하기 위해 만든 WAS (웹 애플리케이션 서버)
...
3. 서블릿에 요청에 대한 실행이 완료된 후에는 서블릿 컨테이너가 실행 결과를 웹서버에 전달한다.
...
- 배포 서술자의 정보를 가지고 웹 커넽이너가 서블릿 구동한다.

### 서블릿 매핑

- 사용자의 요청을 서블릿에게 전달하기 위해서는 서블릿을 등록하고 매핑해야 한다.
- 매핑방법은 web.xml과 @annotation을 이용하는 방법이 있다.
...
- getParameter(String) : client가 보내준 값이저장된 명칭이 매개변수와 같은 명칭에 저장된 값을 불러오는 메소드
...
- Servlet : Java 코드에 HTML 코드가 삽입, 비즈니스 로직 처리에 적합
